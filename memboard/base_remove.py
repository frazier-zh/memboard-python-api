...
class board:
    ...
    def open():
        FrontPanel.open()
        FrontPanel.load('./verilog/src/top.bit')

    def close():
        FrontPanel.close()

    def reset():
        FrontPanel.trigger_in(0x40, 0) # Reset logic block
        FrontPanel.trigger_in(0x40, 1) # Reset memory block
        FrontPanel.trigger_in(0x40, 2) # Reset fifo blocks

    def start():
        FrontPanel.wire_in(0x00, 0b01)

    def start_auto():
        FrontPanel.wire_in(0x00, 0b11) # Enable execution

    def wait_stop(time_out=1):
        start_time = time.time()
        while not board.get_state(dev.logic) == state.idle:
            if (time.time()-start_time > time_out):
                FrontPanel.trigger_in(0x40, 0)
                raise TimeoutError('Time out while waiting on main logic.')
        board.stop()

    def stop():
        FrontPanel.wire_in(0x00, 0b00) # Stop execution

    def set_prog(code):
        FrontPanel.pipe_in(0x80, u.to_byte(code)) # Load program

    def set_clock(every):
        FrontPanel.pipe_in(0x81, u.to_byte(u.to_tick(every), 6))# Load clock counter

    def get_count():
        return FrontPanel.wire_out(0x20) # Check cycle count

    def get_state(target=0):
        FrontPanel.update_wire_out()
        state1 = FrontPanel.read_wire_out(0x21)
        state2 = FrontPanel.read_wire_out(0x22)
    ...
    def get_output(size): # Size in 2-bytes (int16)
        ...
        FrontPanel.pipe_out(0xA0, result)
        return result
